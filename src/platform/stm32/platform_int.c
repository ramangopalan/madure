...
static int int_tmr_match_set_status( picoc_int_resnum resnum, int status )
{
  return PLATFORM_INT_NOT_HANDLED;
}

static int int_tmr_match_get_status( picoc_int_resnum resnum )
{
  return PLATFORM_INT_NOT_HANDLED;
}

static int int_tmr_match_get_flag( picoc_int_resnum resnum, int clear )
{
  return PLATFORM_INT_NOT_HANDLED;
}
...
static int int_uart_rx_get_status( picoc_int_resnum resnum )
{
  return USART_GetITStatus( stm32_usart[ resnum ], USART_IT_RXNE ) == SET ? 1 : 0;
}

static int int_uart_rx_set_status( picoc_int_resnum resnum, int status )
{
...
static int int_uart_rx_get_flag( picoc_int_resnum resnum, int clear )
{
...
const picoc_int_descriptor picoc_int_table[ INT_PICOC_LAST ] = 
{

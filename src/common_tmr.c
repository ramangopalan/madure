...
#if defined( BUILD_PICOC_INT_HANDLERS ) || defined( BUILD_C_INT_HANDLERS )
...
#if defined( BUILD_INT_HANDLERS ) && ( INT_TMR_MATCH != PICOC_INT_INVALID_INTERRUPT )
#define CMN_TIMER_INT_SUPPORT
#endif // #if defined( BUILD_INT_HANDLERS ) && ( INT_TMR_MATCH != PICOC_INT_INVALID_INTERRUPT )
  ...
  for( i = 0; i < VTMR_NUM_TIMERS; i ++ )
  {
    ...
    if( vtmr_counters[ i ] >= vtmr_period_limit[ i ] )
    {
      ...
      if( vtmr_int_enabled[ i >> 3 ] & msk )      
        picoc_int_add( INT_TMR_MATCH, i + VTMR_FIRST_ID );
...
static int vtmr_int_get_flag( picoc_int_resnum resnum, int clear )
{
...
static int vtmr_int_set_status( picoc_int_resnum resnum, int status )
{
  unsigned id = VTMR_GET_ID( resnum );
  u8 msk = 1 << ( id & 0x07 );
...
static int vtmr_int_get_status( picoc_int_resnum resnum )
{
  unsigned id = VTMR_GET_ID( resnum );
  u8 msk = 1 << ( id & 0x07 );
...
int cmn_tmr_int_set_status( picoc_int_resnum resnum, int status )
{
  ...
  if( ( ps = picoc_int_table[ INT_TMR_MATCH - PICOC_INT_FIRST_ID ].int_set_status ) == NULL )
...
int cmn_tmr_int_get_status( picoc_int_resnum resnum )
{
  ...
  if( ( pg = picoc_int_table[ INT_TMR_MATCH - PICOC_INT_FIRST_ID ].int_get_status ) == NULL )
    return PLATFORM_INT_NOT_HANDLED;
  return pg( resnum );
}
...
int cmn_tmr_int_get_flag( picoc_int_resnum resnum, int clear )
{
  ...
  if( ( pf = picoc_int_table[ INT_TMR_MATCH - PICOC_INT_FIRST_ID ].int_get_flag ) == NULL )
    return PLATFORM_INT_NOT_HANDLED;
...
int platform_timer_set_match_int( unsigned id, u32 period_us, int type )
{
  fprintf( stderr, "Timer match interrupt not available when picoc interrupt support is not enabled.\n" );
  return 0;
}

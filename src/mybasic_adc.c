...
picoc_adc_ch_state adc_ch_state[ NUM_ADC ];
picoc_adc_dev_state  adc_dev_state;

picoc_adc_ch_state *adc_get_ch_state( unsigned id )
{
  return &adc_ch_state[ id ];
}

picoc_adc_dev_state *adc_get_dev_state( unsigned dev_id )
{
...
{
  picoc_adc_dev_state *d = adc_get_dev_state( dev_id );
  picoc_adc_ch_state *s;
